...
После того как создадите репозиторий, необходимо изменть origin в проекте, который вы склонировали с репозитория практической. В консоле, находясь в склоненом репозитории, вызвать следующую команду ```git remote set-url origin <ссылка на созданный репозиторий>```. После чего, запустить команду ```git push```
