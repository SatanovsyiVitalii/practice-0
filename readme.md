...
В консоле разработчика должна повиться вкладка Redux
...
По это ссылке https://git-scm.com/downloads можно установить git на ваш компьютер, под вашу операционную систему. 

2. Проверить работу git можно открыв терминал, и введя команду ```git --version```. В консоле должна отобразиться версия вашего git.

3. Клонирование репозитория

Для того, чтобы склонировать проект необходимо перейти по ссылке в репозиторий (https://github.com/SatanovsyiVitalii/practice-0-app). Скопировать SSH ссылку.

![alt text](git-1.png "Title")

Затем открыть терминал, перейти в папку, в которую хотите склонировать проект, и ввести команду ```git clone git@github.com:SatanovsyiVitalii/practice-0-app.git```

4. Запуск приложения.

Для того, чтобы запустить приложение необходимо установить зависимости. Перейдите в папку склоненого проекта, и запустите команду ```npm i```

5. Изменение origin

Создайте аккаунт в GitHub, если еще не сделали этого. Затем создайте репозиторий для 0-й практической (для всех остальных то же самое). Создавайте ```public``` репозиторий.

![alt text](git-2.png "Title")

После того как создадите репозиторий, необходимо изменть origin в проекте, который вы склонировали с репозитория практической. В консоле, находясь в папке со склоненым проектом, вызвать следующую команду ```git remote set-url origin <ssh ссылка на созданный вами репозиторий для практической>```. После чего, запустить команду ```git push```

6. Заливка изменений на GitHub

После того как вы внесли изменеия в проект, необходимо их залить на ваш репозиторий. Для этого необходимо проиндексировать файлы (чтобы гит знал что ему пушить), с помощью команды ```git add .```, закоммититься ```git commit -m "Text for commit"```, и запушить ```git push```