...
После того как создадите репозиторий, необходимо изменть origin в проекте, который вы склонировали с репозитория практической. В консоле, находясь в папке со склоненым проектом, вызвать следующую команду ```git remote set-url origin <ссылка на созданный репозиторий>```. После чего, запустить команду ```git push```

6. Заливка изменений на GitHub

После того как вы внесли изменеия в проект необходимо из залить на ваш репозиторий. Для этого необходимо проиндексировать файлы (чтобы гит знал что ему пушить), с помощью команды ```git add .```, закоммититься ```git commit -m "Text for commit"```, и запушить ```git push```